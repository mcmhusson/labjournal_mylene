...
# Initialize main list for collaborations
all_colabs <- list()
...
for (author in names(all_author_data)) {
  ...
  # fetch the publications safely
  res <- GET(works_url)
  
  # check if response is JSON
  if (http_type(res) != "application/json") {
    warning(paste("Author", author, "did not return JSON. Skipping."))
    all_colabs[[author]] <- list()  # store empty list to keep structure
    next
  }
  
  # parse JSON content
  publi <- fromJSON(content(res, "text"), simplifyVector = FALSE)
  
  # if no results, store empty list and continue
  if (length(publi$results) == 0) {
    all_colabs[[author]] <- list()
    next
  }
